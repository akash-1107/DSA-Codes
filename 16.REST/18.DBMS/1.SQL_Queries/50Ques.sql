...
select INSTR(first_name, 'B') from worker where first_name = 'Amitabh';  --(instr is case insensitive)
...
select w.* from worker as w inner join title as t on w.worker_id = t.worker_ref_id where t.worker_title = 'Manager'; --(w.* mtlb join ke baad bss w ka hi chij show kro)
